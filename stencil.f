...
#else
      ! Note: need to use contiguous here because the matrices get passed into a kernel. (Can't use intent(inout) here).
      real, pointer, contiguous :: a(:,:)
      real, pointer, contiguous :: b(:,:)
      real, pointer, contiguous :: temp(:,:) ! used for pointer swapping (not used right now).
#endif
...
#else
      real, pointer, contiguous :: topBoundary(:)
      real, pointer, contiguous :: bottomBoundary(:)
      real, pointer, contiguous :: topHalo(:)
      real, pointer, contiguous :: bottomHalo(:)
#endif
      integer :: msgSize
...
#ifdef USE_ALLOCATABLE
! !$ACC data copy(a(1:ht, 1:N+2)) create(b(2:ht-1, 2:N+1))
...
! TODO: Figure out whether the below should operate on 1, N+2 or 2, N+1.
           ...
           if (rank == (size-1)) then
              !$ACC kernels loop copy(bottomHalo(1:N+2)) pcopy(a(ht-1,1:N+2))
              do i = 1, N+2
...
!$ACC kernels present(a(1:ht, 1:N+2)) pcreate(b(2:ht-1, 2:N+1)) copyin(topHalo(1:N+2), bottomHalo(1:N+2)) copyout(topBoundary(1:N+2), bottomBoundary(1:N+2)) pcopyin(rank, size)
        ...
        ! reset boundaries
           ...
           !TODO: may need to have a single GPU thread do this.
           if(rank == 0) then
              a(1,2) = 0.0
              a(2,1) = 0.0
           end if
...
!$ACC loop seq
        ...
        !TODO: make this consistent with functionized version
